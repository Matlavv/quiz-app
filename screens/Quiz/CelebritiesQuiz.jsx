  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch(
        `https://opentdb.com/api.php?amount=10&category=26&difficulty=${difficulty}&type=multiple`
      );
      ...
      if (json.results && json.results.length > 0) {
        ...
        throw new Error("No results or invalid response structure");
      }
    } catch (error) {
    ...
    <View className="flex-1 bg-primary-purple">
      ...
      <View className="flex-1 p-4 m-2 bg-white rounded-2xl">
        <View className="flex items-center justify-center">
          <Text className=" text-2xl font-bold text-black">Célébrités</Text>
        </View>
